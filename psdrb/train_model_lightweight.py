...
pairs_df = create_pairs_auto(top_thresh=0.6, low_thresh=0.20, max_pairs_per_doc=3)
...
def pair_features(i, j):
    """Ekstraksi 9 fitur untuk pasangan (i, j)"""
    ...
    # 5. Levenshtein similarity (edit distance)
    lev = levenshtein_ratio(df.loc[i, 'Judul'], df.loc[j, 'Judul'])
    
    # 6-9. Statistik perbedaan absolut TF-IDF
    diff = np.abs(vi - vj)
...
print("\n[7/8] Training LIGHTWEIGHT models...")
print("\n--- Training Linear SVM (smaller model size) ---")
# Linear kernel lebih kecil dari RBF
param_grid_svm = {
    'C': [0.1, 1],  # Reduced search space
    'kernel': ['linear']  # Linear kernel = smaller model
}
...
y_proba_svm = best_svm.predict_proba(X_test_s)[:, 1]
print("\nSVM Test Results:")
print(classification_report(y_test, y_pred_svm, target_names=['Tidak Mirip', 'Mirip']))
...
joblib.dump(best_svm, f"{output_dir}/best_svm.joblib", compress=3)
...
for fname in ['tfidf.joblib', 'scaler.joblib', 'best_svm.joblib', 'best_rf.joblib', 'titles_preprocessed.csv']:
